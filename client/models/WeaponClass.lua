...
local function RDApplyWeaponComponent(playerPedId, weaponName, value, seen)
	if value == nil then return end
	seen = seen or {}
	local valueType = type(value)
	if valueType == "table" then
		for k, v in pairs(value) do
			if type(k) == "string" and k ~= "" and k ~= "nothing" and (v == true or v == 1 or v == "1") then
				RDApplyWeaponComponent(playerPedId, weaponName, k, seen)
			else
				RDApplyWeaponComponent(playerPedId, weaponName, v, seen)
			end
		end
		return
	end

	if value == "" or value == "nothing" or value == 0 then return end
	local componentHash = valueType == "number" and value or joaat(tostring(value))
	if not componentHash or componentHash == 0 or seen[componentHash] then return end
	seen[componentHash] = true
	Citizen.InvokeNative(0x74C9090FDD1BB48E, playerPedId, componentHash, joaat(weaponName), true)
end

function Weapon:loadComponents()
	local playerPedId = PlayerPedId()
	RDApplyWeaponComponent(playerPedId, self.name, self.components or {})
end
