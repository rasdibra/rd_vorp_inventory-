	...
	if item then
		...
		if item:getMaxDegradation() == 0 then
			...
			if result and doesMetadataExist then
				...
				if not allow and RDItemNotify then
					RDItemNotify.Add(_source, name, amount, metadata or {}, { label = RDItemNotify.GetLabel(name, metadata or {}, name) })
				end
			...
			if itemNoMetadata then
				...
				if not allow and RDItemNotify then
					RDItemNotify.Add(_source, name, amount, {}, { label = RDItemNotify.GetLabel(name, {}, name) })
				end
				return respond(cb, true)
			end
		end
	...
	DBService.CreateItem(charIdentifier, svItem:getId(), amount, metadata or {}, name, isExpired, function(craftedItem)
		...
		if not allow then
			...
			if RDItemNotify then
				RDItemNotify.Add(_source, newItem:getName(), amount, newItem:getMetadata(), { label = RDItemNotify.GetLabel(newItem:getName(), newItem:getMetadata(), newItem:getLabel()) })
			end
		end
...
function InventoryAPI.subItemID(player, id, cb, allow, amount)
	...
	if not allow then
		...
		if RDItemNotify then
			RDItemNotify.Remove(_source, itemName, amount, itemMetadata, { label = RDItemNotify.GetLabel(itemName, itemMetadata, itemName) })
		end
	end
	return respond(cb, true)
end
	...
	if exactMatchItem then
		...
		for _, value in ipairs(itemsToRemove) do
			...
			if not allow then
				...
				TriggerEvent("vorp_inventory:Server:OnItemRemoved", data, _source)
				if RDItemNotify then
					RDItemNotify.Remove(_source, itemName, removeCount, itemMetadata, { label = RDItemNotify.GetLabel(itemName, itemMetadata, itemName) })
				end
			end
		end
	end
...
function InventoryAPI.registerWeapon(_target, wepname, ammos, components, comps, cb, wepId, customSerial, customLabel, customDesc)
	...
	DBService.insertAsync(query, params, function(result)
		...
		TriggerClientEvent("vorpInventory:receiveWeapon", _target, weaponId, targetIdentifier, name, ammo, label, serialNumber, label, _target, desc, weight, comps or component or {})
	end)
	return respond(cb, true)
end
...
function InventoryAPI.giveWeapon(player, weaponId, target, cb)
	...
	if weapon then
		...
		DBService.updateAsync(query, params, function(r)
			...
			TriggerClientEvent("vorpInventory:receiveWeapon", _source, weaponId, weaponPropietary, weaponName, weaponAmmo, label, serialNumber, customLabel, _source, customDesc, weight, weapon:getAllComponents())
		end)
	end
	return respond(cb, true)
end
