...
local function RDStorageCleanStackEnabled()
	return not Config.RDStorageCleanStack or Config.RDStorageCleanStack.Enabled ~= false
end

local function RDStorageIgnoreStackLimit(invId)
	return RDStorageCleanStackEnabled()
		and invId ~= "default"
		and (not Config.RDStorageCleanStack or Config.RDStorageCleanStack.IgnoreStackLimitInStorage ~= false)
end
...
function InventoryService.giveMoneyToPlayer(target, amount)
	...
	if RDItemNotify then
		RDItemNotify.Send(_source, 'money_remove', 'cash', amount, {}, { label = T.inventorymoneylabel or 'Cash', imageOverride = 'img/cash.png' })
		RDItemNotify.Send(_target, 'money_add', 'cash', amount, {}, { label = T.inventorymoneylabel or 'Cash', imageOverride = 'img/cash.png' })
	end
	if not RDItemNotify or RDItemNotify.LegacyMovementEnabled() then
...
function InventoryService.giveGoldToPlayer(target, amount)
	...
	if RDItemNotify then
		RDItemNotify.Send(_source, 'gold_remove', 'gold', amount, {}, { label = T.inventorygoldlabel or 'Gold', imageOverride = 'img/gold.png' })
		RDItemNotify.Send(target, 'gold_add', 'gold', amount, {}, { label = T.inventorygoldlabel or 'Gold', imageOverride = 'img/gold.png' })
	end
	if not RDItemNotify or RDItemNotify.LegacyMovementEnabled() then
	...
	if item then
		...
		if amount > 0 then
			...
			if svItem:getMaxDegradation() == 0 then
				local rdIgnoreStackLimit = CustomInventoryInfos[invId].ignoreItemStackLimit or RDStorageIgnoreStackLimit(invId)
					local success = item:addCount(amount, rdIgnoreStackLimit)
				if not success then
					return cb(false)
				end
...
function InventoryService.onPickup(data)
	local _source <const> = source
	data = data or {}
	local keepInventoryOpen <const> = data.fromNui == true or data.keepInventoryOpen == true
	...
	if not userInventory then
		SvUtils.Trem(_source, keepInventoryOpen)
		return
	end

	-- is item
	if pickup.weaponid == 1 then
		local requested = tonumber(data.amount or data.pickupAmount or pickup.amount) or pickup.amount
		requested = math.floor(requested)
		if requested < 1 then requested = 1 end
		if requested > pickup.amount then requested = pickup.amount end

		local canCarryWeight <const> = InventoryAPI.canCarryAmountItem(_source, requested)
		local canCarryLimit <const> = InventoryAPI.canCarryItem(_source, pickup.name, requested)
		...
		InventoryService.addItem(_source, "default", pickup.name, requested, pickup.metadata, info, function(item)
			if item and ItemPickUps[uid] then
				local livePickup = ItemPickUps[uid]
				local remaining = (livePickup.amount or pickup.amount or requested) - requested
				local obj = data.obj or livePickup.obj or pickup.obj

				if remaining > 0 then
					livePickup.amount = remaining
					local updateData = {
						obj = obj,
						uid = uid,
						name = livePickup.name or pickup.name,
						amount = remaining,
						metadata = livePickup.metadata or pickup.metadata,
						degradation = livePickup.degradation or pickup.degradation,
						position = livePickup.coords or pickup.coords,
						type = livePickup.type or "item_standard",
					}
					TriggerClientEvent("vorpInventory:sharePickupClient", -1, updateData, 3)
				else
					ItemPickUps[uid] = nil
					TriggerClientEvent("vorpInventory:sharePickupClient", -1, { obj = obj }, 2)
				end

				TriggerClientEvent("vorpInventory:receiveItem", _source, pickup.name, item:getId(), requested, pickup.metadata, item.degradation, item.percentage)
				if RDItemNotify then
					RDItemNotify.Send(_source, 'pickup', pickup.name, requested, pickup.metadata, { label = RDItemNotify.GetLabel(pickup.name, pickup.metadata, pickup.name) })
				end
				TriggerClientEvent("vorpInventory:playerPickUpAnim", _source, uid)

				local charname <const>, _, steamname <const> = getSourceInfo(_source)
				local title <const>                          = T.itempickup
				local description <const>                    = "**" .. T.WebHookLang.amount .. "** `" .. requested .. "`\n **" .. T.WebHookLang.item .. "** `" .. pickup.name .. "` \n**" .. T.WebHookLang.charname .. ":** `" .. charname .. "`\n**" .. T.WebHookLang.Steamname .. "** `" .. steamname .. "`"
		...
		if weapon then
			...
			if DefaultAmount ~= 0 then
				...
				if totalInvWeight <= invCapacity or sourceInventoryWeaponCount <= DefaultAmount then
					...
					if RDItemNotify then
						RDItemNotify.Send(_source, 'pickup', weaponName, 1, {}, { label = RDItemNotify.GetLabel(weaponName, {}, weaponName) })
					end
					SvUtils.SendDiscordWebhook(info)
				end
			else
...
function InventoryService.onPickupMoney(data)
	...
	data = data or {}
	local keepInventoryOpen <const> = data.fromNui == true or data.keepInventoryOpen == true
	local requestAmount = tonumber(data.amount or data.pickupAmount)

	local uuid = data.uuid
	local pickup = MoneyPickUps[uuid]
	if not pickup then return end

	if not SvUtils.InProcessing(_source) then
		SvUtils.ProcessUser(_source)

		local moneyAmount = pickup.amount
		local takeAmount = requestAmount or moneyAmount
		if takeAmount <= 0 then
			SvUtils.Trem(_source, keepInventoryOpen)
			return
		end
		if takeAmount > moneyAmount then takeAmount = moneyAmount end
		local remaining = moneyAmount - takeAmount

		local title = T.WebHookLang.moneypickup
		local description = "**" .. T.WebHookLang.money .. ":** `" .. takeAmount .. "` `$` \n**" .. T.WebHookLang.charname .. ":** `" .. charname .. "`\n**" .. T.WebHookLang.Steamname .. "** `" .. steamname .. "`\n"
		...
		character.addCurrency(0, takeAmount)
		if RDItemNotify then
			RDItemNotify.Send(_source, 'money_add', 'cash', takeAmount, {}, { label = T.inventorymoneylabel or 'Cash', imageOverride = 'img/cash.png' })
		end

		if remaining > 0 then
			pickup.amount = remaining
			TriggerClientEvent("vorpInventory:shareMoneyPickupClient", -1, pickup.obj, remaining, pickup.coords, pickup.uuid, 3)
		else
			TriggerClientEvent("vorpInventory:shareMoneyPickupClient", -1, pickup.obj, nil, nil, nil, 2)
			MoneyPickUps[uuid] = nil
		end

		TriggerClientEvent("vorpInventory:playerPickUpAnim", _source, pickup.obj)
		SvUtils.Trem(_source, keepInventoryOpen)
	end
end
...
function InventoryService.onPickupGold(data)
	...
	local requestAmount = tonumber(data.amount or data.pickupAmount)
	local uuid = data.uuid
	local pickup = GoldPickUps[uuid]
	if not pickup then return end

	if not SvUtils.InProcessing(_source) then
		SvUtils.ProcessUser(_source)

		local goldAmount = pickup.amount
		local takeAmount = requestAmount or goldAmount
		if takeAmount <= 0 then
			SvUtils.Trem(_source, keepInventoryOpen)
			return
		end
		if takeAmount > goldAmount then takeAmount = goldAmount end
		local remaining = goldAmount - takeAmount

		local character = Core.getUser(_source).getUsedCharacter
		local charname, _, steamname = getSourceInfo(_source)
		local title = T.WebHookLang.pickedgold
		local description = "**" .. T.WebHookLang.gold .. "** `" .. takeAmount .. "` \n**" .. T.WebHookLang.charname .. ":** `" .. charname .. "`\n**" .. T.WebHookLang.Steamname .. "** `" .. steamname .. "`\n"
		...
		character.addCurrency(1, takeAmount)
		if RDItemNotify then
			RDItemNotify.Send(_source, 'gold_add', 'gold', takeAmount, {}, { label = T.inventorygoldlabel or 'Gold', imageOverride = 'img/gold.png' })
		end
		SvUtils.SendDiscordWebhook(info)

		if remaining > 0 then
			pickup.amount = remaining
			TriggerClientEvent("vorpInventory:shareGoldPickupClient", -1, pickup.obj, remaining, pickup.coords, pickup.uuid, 3)
		else
			TriggerClientEvent("vorpInventory:shareGoldPickupClient", -1, pickup.obj, nil, nil, nil, 2)
			GoldPickUps[uuid] = nil
		end
...
local function rdPickupCoordsDistance(a, b)
...
local function rdSamePickupMetadata(a, b)
	return SharedUtils.Table_equals(a or {}, b or {}, true)
end

local function shareData(data)
	-- RD FIX: same item drops near each other now merge into one ground slot instead of creating
	-- multiple duplicate slots. This keeps the Ground panel clean and fixes repeated drop glitches.
	if data and data.type == "item_standard" and data.name and tonumber(data.amount or 0) > 0 then
		for uid, pickup in pairs(ItemPickUps) do
			if pickup
				and pickup.name == data.name
				and pickup.weaponid == data.weaponId
				and rdSamePickupMetadata(pickup.metadata, data.metadata)
				and (pickup.degradation == data.degradation or pickup.degradation == nil or data.degradation == nil)
				and rdPickupCoordsDistance(pickup.coords, data.position) <= 3.0
			then
				local amount = (tonumber(pickup.amount) or 0) + (tonumber(data.amount) or 0)
...
function InventoryService.sharePickupServerWeapon(data)
	...
	if RDItemNotify then
		RDItemNotify.Send(_source, 'drop', wepname, 1, {}, { label = RDItemNotify.GetLabel(wepname, {}, wepname) })
	end
	shareData(data)
end
...
function InventoryService.sharePickupServerItem(data)
	...
	if RDItemNotify then
		RDItemNotify.Send(_source, 'drop', data.name, data.amount, data.metadata, { label = RDItemNotify.GetLabel(data.name, data.metadata, data.name) })
	end
...
function InventoryService.shareGoldPickupServer(data)
	...
	if RDItemNotify then
		RDItemNotify.Send(_source, 'gold_remove', 'gold', data.amount, {}, { label = T.inventorygoldlabel or 'Gold', imageOverride = 'img/gold.png' })
	end
	local uid = SvUtils.GenerateUniqueID()
...
function InventoryService.DropWeapon(weaponId)
	...
	if not SvUtils.InProcessing(_source) then
		...
		if not Config.DeleteOnlyDontDrop then
			...
			local removed = InventoryService.subWeapon(_source, weaponId)
			if removed and RDItemNotify then
				RDItemNotify.Send(_source, 'drop', wepName, 1, {}, { label = RDItemNotify.GetLabel(wepName, {}, wepName) })
			end
		end
		SvUtils.Trem(_source)
...
function InventoryService.giveWeapon2(player, weaponId, target)
	...
	if RDItemNotify then
		RDItemNotify.Send(_target, 'give_out', weaponName, 1, {}, { label = RDItemNotify.GetLabel(weaponName, {}, weaponName) })
		RDItemNotify.Send(_source, 'give_in', weaponName, 1, {}, { label = RDItemNotify.GetLabel(weaponName, {}, weaponName) })
	end
end
...
function InventoryService.GiveItem(itemId, amount, target)
	...
	local function updateClient(addedItem)
		...
		if RDItemNotify then
			RDItemNotify.Send(_source, 'give_out', itemName, amount, itemMetadata, { label = label })
			RDItemNotify.Send(_target, 'give_in', itemName, amount, itemMetadata, { label = label })
		end
	end
...
-- RD STORAGE CLEAN STACK:
-- Custom storage/stash inventory limits are treated as SLOT limits, not item-count limits.
-- Existing same item+metadata stacks do not consume a new slot, so players can move 1-by-1
-- into the same slot without hitting a fake stack limit.
function InventoryService.getInventoryTotalSlots(identifier, charIdentifier, invId)
	invId = invId ~= nil and invId or "default"
	local invData = CustomInventoryInfos[invId]
	if not invData then return 0 end
	local slotCount = 0
	local userInventory = {}
	local userWeapons = UsersWeapons[invId] or {}
	if invData:isShared() then
		userInventory = UsersInventories[invId] or {}
	else
		userInventory = (UsersInventories[invId] and UsersInventories[invId][identifier]) or {}
	end
	for _, _ in pairs(userInventory) do
...
function InventoryService.canStoreWeapon(identifier, charIdentifier, invId, name, amount)
	...
	if invData:getLimit() > 0 then
		local sourceInventoryItemCount
		if RDStorageCleanStackEnabled() and invId ~= "default" then
			sourceInventoryItemCount = InventoryService.getInventoryTotalSlots(identifier, charIdentifier, invId) + amount
		else
			sourceInventoryItemCount = InventoryService.getInventoryTotalCount(identifier, charIdentifier, invId) + amount
		end
...
function InventoryService.canStoreItem(identifier, charIdentifier, invId, name, amount, metadata)
	...
	if invData:getLimit() > 0 then
		local sourceInventoryItemCount
		if RDStorageCleanStackEnabled() and invId ~= "default" then
			local existingStack = SvUtils.FindItemByNameAndMetadata(invId, identifier, name, metadata)
			local addSlots = existingStack and 0 or 1
			sourceInventoryItemCount = InventoryService.getInventoryTotalSlots(identifier, charIdentifier, invId) + addSlots
		else
			sourceInventoryItemCount = InventoryService.getInventoryTotalCount(identifier, charIdentifier, invId) + amount
		end
		if sourceInventoryItemCount > invData:getLimit() then
	...
	if not invData:getIgnoreItemStack() and not RDStorageIgnoreStackLimit(invId) then
...
function InventoryService.MoveToCustom(obj)
	...
	if item.type == "item_weapon" then
		...
		if RDItemNotify then
			RDItemNotify.Send(_source, 'storage_put', item.name, 1, {}, { label = RDItemNotify.GetLabel(item.name, {}, item.name) })
		end
		if not RDItemNotify or RDItemNotify.LegacyMovementEnabled() then
		...
		InventoryService.addItem(_source, invId, item.name, amount, item.metadata, info, function(itemAdded)
			...
			if RDItemNotify then
				RDItemNotify.Send(_source, 'storage_put', item.name, amount, item.metadata, { label = metadataLabel })
			end
...
function InventoryService.TakeFromCustom(obj)
	...
	if item.type == "item_weapon" then
		...
		TriggerClientEvent("vorpInventory:receiveWeapon", _source, item.id, sourceIdentifier, name, ammo, label, serial, custom, _source, customDesc, weight, weapon:getAllComponents())
		InventoryService.reloadInventory(_source, invId)
		...
		if RDItemNotify then
			RDItemNotify.Send(_source, 'storage_take', item.name, 1, {}, { label = RDItemNotify.GetLabel(item.name, {}, item.name) })
		end
		if not RDItemNotify or RDItemNotify.LegacyMovementEnabled() then
			Core.NotifyAvanced(_source, text, "inventory_items", Icon, "COLOR_PURE_WHITE", 4000)
		end
		...
		InventoryService.addItem(_source, "default", item.name, amount, item.metadata, info, function(itemAdded)
			...
			if RDItemNotify then
				RDItemNotify.Send(_source, 'storage_take', item.name, amount, itemAdded:getMetadata(), { label = metadataLabel })
			end
			SvUtils.Trem(_source)
		end)
	end
				...
				InventoryAPI.subItem(_source, item.name, amount, item.metadata, function(result2)
					if result2 then
						SetTimeout(400, function()
							...
							if RDItemNotify then
								RDItemNotify.Send(_source, 'player_put', item.name, amount, item.metadata, { label = metadataLabel })
								RDItemNotify.Send(target, 'player_take', item.name, amount, item.metadata, { label = metadataLabel })
							end
							SvUtils.Trem(_source)
						end)
					else
			...
			if result then
				InventoryAPI.subItem(target, item.name, amount, item.metadata, function(result2)
					if result2 then
						...
						if RDItemNotify then
							RDItemNotify.Send(_source, 'player_take', item.name, amount, item.metadata, { label = metadataLabel })
							RDItemNotify.Send(target, 'player_put', item.name, amount, item.metadata, { label = metadataLabel })
						end
					end
					SvUtils.Trem(_source)
				end, true)
