...
local function RDDecodeWeaponComponents(value)
...
local function RDPushWeaponComponent(out, seen, value)
	if value == nil then return end
	local valueType = type(value)
	if valueType == "table" then
		for k, v in pairs(value) do
			if type(k) == "string" and k ~= "" and k ~= "nothing" and (v == true or v == 1 or v == "1") then
				RDPushWeaponComponent(out, seen, k)
			else
				RDPushWeaponComponent(out, seen, v)
			end
		end
		return
	end
...
local function RDMergeWeaponComponents(...)
	local out, seen = {}, {}
	for i = 1, select('#', ...) do
		RDPushWeaponComponent(out, seen, RDDecodeWeaponComponents(select(i, ...)))
	end
	return out
end
...
local function loadAllWeapons(db_weapon)
	...
	local comp = RDMergeWeaponComponents(db_weapon.components, db_weapon.comps)
